...
export function useCollaboration(editorRef: React.MutableRefObject<editor.IStandaloneCodeEditor | null>) {
  const { sessionId, setActiveUsers, updateCursor, addChatMessage } = useCollaborationStore()
  ...
  const handleCursorMove = useCallback((data: { username: string; position: number }) => {
  ...
  const handleChatMessage = useCallback((data: { username: string; message: string }) => {
  ...
  }, [sessionId, handleCodeChange, handleCursorMove, handleUsersChange, handleChatMessage])
